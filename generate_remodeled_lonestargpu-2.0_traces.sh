...
# Usage:
#   ./generate_remodeled_lonestargpu-2.0_traces.sh -D 0
#   ./generate_remodeled_lonestargpu-2.0_traces.sh 0
...
export LONESTAR_TRACE_MAX_ITERATIONS="${LONESTAR_TRACE_MAX_ITERATIONS:-50}"
export LONESTAR_TRACE_SKIP_VERIFY="${LONESTAR_TRACE_SKIP_VERIFY:-1}"
...
echo "Benchmark suite=lonestargpu-2.0"
echo "Build target=lonestargpu-2.0"
...
echo "Lonestar trace iteration limit=$LONESTAR_TRACE_MAX_ITERATIONS (0 means unlimited)"
echo "Skip Lonestar GPU verification=$LONESTAR_TRACE_SKIP_VERIFY (0 restores verification)"
...
make -j"$(nproc)" -C ./gpu-app-collection/src lonestargpu-2.0
make -C ./gpu-app-collection/src data

./util/tracer_nvbit/run_hw_trace.py -B lonestargpu-2.0 -D "$DEVICE"
