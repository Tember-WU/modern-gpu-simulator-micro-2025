...
# Usage:
#   ./generate_remodeled_polybench_traces.sh -D 0
#   ./generate_remodeled_polybench_traces.sh 0
...
# Limit the two PolyBench applications whose host loops otherwise create very
# large traces. Both variables may be overridden by the caller.
export POLYBENCH_GRAMSCHMIDT_ITERATIONS="${POLYBENCH_GRAMSCHMIDT_ITERATIONS:-100}"
export POLYBENCH_FDTD_ITERATIONS="${POLYBENCH_FDTD_ITERATIONS:-100}"
...
echo "Benchmark suite=polybench"
echo "Build target=polybench"
...
echo "PolyBench Gram-Schmidt iterations=$POLYBENCH_GRAMSCHMIDT_ITERATIONS"
echo "PolyBench FDTD iterations=$POLYBENCH_FDTD_ITERATIONS"
...
make -j"$(nproc)" -C ./gpu-app-collection/src polybench
make -C ./gpu-app-collection/src data

./util/tracer_nvbit/run_hw_trace.py -B polybench -D "$DEVICE"
