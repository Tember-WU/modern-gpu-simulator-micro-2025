...
# Usage:
#   ./generate_remodeled_dragon-naive_traces.sh -D 0
#   ./generate_remodeled_dragon-naive_traces.sh 0
...
if [ ! -d "$BOOST_ROOT/include/boost" ]; then
    echo "ERROR: Boost headers were not found under $BOOST_ROOT/include." >&2
    exit 1
fi
...
echo "Benchmark suite=dragon-naive"
echo "Build target=dragon-naive"
...
make -j"$(nproc)" -C ./gpu-app-collection/src dragon-naive
make -C ./gpu-app-collection/src data

./util/tracer_nvbit/run_hw_trace.py -B dragon-naive -D "$DEVICE"
