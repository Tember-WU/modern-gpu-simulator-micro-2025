...
# Usage:
#   ./generate_remodeled_pannotia_traces.sh -D 0
#   ./generate_remodeled_pannotia_traces.sh 0
...
export PANNOTIA_BC_SOURCES="${PANNOTIA_BC_SOURCES:-20}"
...
echo "Benchmark suite=pannotia"
echo "Build target=pannotia"
...
echo "Pannotia BC sources=$PANNOTIA_BC_SOURCES"
...
make -j"$(nproc)" -C ./gpu-app-collection/src pannotia
make -C ./gpu-app-collection/src data

./util/tracer_nvbit/run_hw_trace.py -B pannotia -D "$DEVICE"
