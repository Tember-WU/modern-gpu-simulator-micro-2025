...
# Do not enable `set -u`: gpu-app-collection's legacy setup script reads
# optional variables before defining them.
set -eo pipefail

# Usage:
#   ./generate_remodeled_parboil_traces.sh -D 0
#   ./generate_remodeled_parboil_traces.sh 0
...
# Build all Parboil applications and download their input datasets.
source ./gpu-app-collection/src/setup_environment
make -j"$(nproc)" -C ./gpu-app-collection/src parboil
make -C ./gpu-app-collection/src data

# Generate traces for every workload in the `parboil` suite.
./util/tracer_nvbit/run_hw_trace.py -B parboil -D "$DEVICE"
