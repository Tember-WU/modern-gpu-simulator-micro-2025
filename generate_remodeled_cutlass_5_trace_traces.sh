...
# Usage:
#   ./generate_remodeled_cutlass_5_trace_traces.sh -D 0
#   ./generate_remodeled_cutlass_5_trace_traces.sh 0
...
# cutlass_perf_test launches the selected GEMM three times even with
# --iterations=1: initial execution, warm-up, then the timed iteration.  They
# are the same static kernel.  Capture the timed launch only so each workload
# keeps one representative dynamic trace plus its complete static artifacts.
export DYNAMIC_KERNEL_LIMIT_START="${DYNAMIC_KERNEL_LIMIT_START:-3}"
...
echo "Benchmark suite=cutlass_5_trace"
echo "Build target=cutlass"
...
make -j"$(nproc)" -C ./gpu-app-collection/src cutlass
make -C ./gpu-app-collection/src data

./util/tracer_nvbit/run_hw_trace.py -B cutlass_5_trace -D "$DEVICE"
